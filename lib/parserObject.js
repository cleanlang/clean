...
const updateParsed = (val, name) => {
  ...
  const maxColumn =
    parser.unParsed.line === val.line &&
    parser.unParsed.column < val.column; // if lines match then compare column
  ...
  if (maxColumn) {
    if (isUndefined(parser.unParsed.str))
      parser.unParsed.str = currentExpr;
    if (name !== "spaceRegex" && name !== "equalSignRegex") {
      parser.unParsed.regex.push(name);
    }

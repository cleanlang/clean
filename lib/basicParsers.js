...
const nonReservedIdParser = input => maybe(
  idParser(input),
  (name, rest) => (isLanguageConstruct(name) || isStaticIOMethod(name) || isIOMethod(name) || isDOMmethod(name))
    ? null
...
const unaryOperatorParser = input => maybe(
  parser.regex(unaryOperatorRegex)(input),
...
const deleteKeywordParser = input => maybe(
  parser.regex(deleteRegex)(input),
  (operator, rest) => [operator, rest]
)

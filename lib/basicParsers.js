...
const reverseBindParser = (input) =>
  parser.regex(reverseBindRegex)(input)
...
const returnKeywordParser = (input) =>
  parser.regex(returnKeywordRegex)(input)

const includeKeywordParser = (input) =>
  parser.regex(includeKeywordRegex)(input)

const definePropParser = (input) =>
  parser.regex(definePropRegex)(input)

const returnParser = (input) =>
  maybe(returnRegex().exec(input.str), (m, newLine, rest) =>
    returnRest(newLine, input, rest, {
      name: 'return',
      value: newLine.length
    })
  )

const numberParser = (input) =>
  maybe(numParser(input), (num, rest) => [
    estemplate.literal(num),
    rest
  ])
...
const regexParser = (input) =>
  maybe(
    ...
    (m, regex, pattern, b, flags, _, rest) =>
...
const commaParser = (input) =>
  maybe(parser.regex(commaRegex)(input), (comma, rest) => [
    comma,
    rest
  ])

const colonParser = (input) =>
  maybe(parser.regex(colonRegex)(input), (colon, rest) => [
    colon,
    rest
  ])
...
const deleteKeywordParser = (input) =>
  maybe(parser.regex(deleteRegex)(input), (operator, rest) => [
    operator,
    rest
  ])

...
const indentStacker = require('./indentStacker')
...
var booleanParser = str => mayBe(
  boolRegex.exec(str),
  (a, bool, rest) => [estemplate.boolLiteral(bool), rest]
)
...
var argsParser = (str, argArray = []) => {
  var arg = parser.bind(expressionParser, val => str => {
    var space = spaceParser(str)
    return space ? [val, space[1]] : [val, str]
  })(str)
  var end = parser.all(expressionParser, parser.any(returnParser, parenCheck))(str)
  if (end) end = [end[0][0], end[1]]
  if (arg === null && end === null) return argArray.length === 0 ? null : [argArray, str]
  if (binaryOperatorRegex.exec(arg[1]) !== null) arg[1] = ' ' + arg[1]
  if (end !== null) arg = end
  if (arg[0].type === 'ExpressionStatement') arg[0] = arg[0].expression
  if (end !== null) return [argArray.concat(arg[0]), arg[1]]
...
var ifExprParser = parser.bind(
  ...
  parser.bind(
    ...
    val => str => mayBe(
      parser.all(spaceParser, thenParser)(str),
      (v, rest) => [val[1], rest]
    )),
    val => str => mayBe(
      parser.any(binaryExprParser, fnCallParser, expressionParser)(str),
    ...
    val => str => mayBe(
      parser.all(spaceParser, elseParser)(str),
      (v, rest) => [val, rest]
    )),
    val => str => mayBe(
      parser.any(binaryExprParser, fnCallParser, expressionParser)(str),
      (v, rest) => [estemplate.ifthenelse(val[0], val[1], v), rest]
    )
)

var indentParser = (str, indentObject = []) => {
  for (let i = 0; i < str.length; i++) {
    let space = spaceParser(str[i])
    if (space !== null) space = space[0].length
    indentObject[i] = {'indent': space || 0,
                          'expr': str[i]}
  }
  return indentObject
}

var multiLineParser = (str, indentObject) => {
  for (let i = 0; i < str.length - 1; i++) {
    if (indentObject[i].indent < indentObject[i + 1].indent) {
      let lead
      if (indentObject[i].indent > 0) lead = multiLineParserFactory(spaceParser(str[i])[1])
      else lead = multiLineParserFactory(str[i])
      const follow = multiLineParserFactory(spaceParser(str[i + 1])[1])
      if (lead === null || follow === null) {
        str[i] = str[i] + str[i + 1]
        str.splice(i + 1, 1)
        indentObject[i].expr = str[i]
        indentObject.splice(i + 1, 1)
        i--
      }
    }
  }
...
var statementParser = parser.any(returnParser, spaceParser, printParser, ifExprParser, letExpressionParser, declParser, fnDeclParser, fnCallParser, lambdaParser)
var multiLineParserFactory = parser.any(returnParser, printParser, ifExprParser, letExpressionParser, declParser, fnDeclParser, fnCallParser, lambdaParser)

var programParser = (str, ast = estemplate.ast(), indentObject) => {
  if (indentObject === undefined) {
    const exprList = str.trim().split('\n')
    var [str, indentObject] = multiLineParser(exprList, indentParser(exprList))
  }
  let result = statementParser(str)
  if (result === null) return updateAst(indentStacker(ast, indentObject))
  if (typeof result[0] !== 'string') ast.body.push(result[0])
  return programParser(result[1], ast, indentObject)
}

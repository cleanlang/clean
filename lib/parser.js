...
const parenCheck = str => str[0] === ')' ? [')', str] : null
...
var declParser = parser.bind(
  ...
  val => input => mayBe(
        ...
        (v, rest) => returnRest(estemplate.declaration(val, v), input, rest.str)
      )
)
...
var argsParser = (input, argArray = []) => {
  ...
  var arg = parser.all(
              ...
              parser.any(returnParser, parenCheck))(input) ||
            parser.bind(
              parser.any(unaryExprParser, expressionParser),
              val => input => {
                const space = spaceParser(input)
...
var statementParser = parser.any(returnParser, printParser, letExpressionParser, declParser, fnDeclParser, fnCallParser, lambdaParser)
...
var programParser = (input, ast = estemplate.ast()) => {
  ...
  if (result[0].interrupt !== undefined && result[0].interrupt === true) {
    ast.body.push(result[0])
  ...
  var descender = s => {
    ...
    if (token === null) {
      if (topOfOpStack !== '$') {
        ...
        operandStack.push(biExpr(leftOperand, operator, rightOperand, rest.line))
        // rest.str = '\n' + rest.str
        return descender(rest)
      }
      return [operandStack.length === 0 ? null : operandStack.pop(), rest]
    }

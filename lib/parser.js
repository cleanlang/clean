...
const paramsParser = (input, idArray = []) =>
  maybe(parser.all(nonReservedIdParser, spaceParser)(input), (val, rest) => {
    let [val_] = val;
...
const arrayParser = (input) => {
  ...
  let closeSquareBracket = parser.all(
    maybeNewLineAndIndent,
    closeSquareBracketParser
  )(arrayPropsRest);
...
const objectParser = (input) => {
  ...
  let closeCurlyResult = parser.all(
    maybeSpace,
    maybeNewLineAndIndent,
    closeCurlyBraceParser
  )(objPropsRest);
...
=======
  ...
  let [exp, rest] = prop;
  if (exp.isSubscript)
    return formMemberExpression(rest, estemplate.subscriptExpression(obj, exp));
  exp.isSubscript = false;
  if (exp.type === "Identifier")
    return formMemberExpression(rest, estemplate.memberExpression(obj, exp));
  return formMemberExpression(rest, obj);
};
...
const memberExprParser = (input) =>
  maybe(
    ...
    (val, rest) => {
      ...
      let [memExpr, _rest] = result;
      memExpr =
        memExpr.type === "ExpressionStatement" ? memExpr.expression : memExpr;
      return memExpr.type === "MemberExpression" ? [memExpr, _rest] : null;
    }
  );
...
>>>>>>> add git-hooks using husky and lint-staged to format with prettier
...
const ioStmtParser = (
  ...
  maybe(ioFuncName(input), (val, rest) => {
    ...
    let callBack = isEmptyArr(mapBody.stmts)
      ? estemplate.lambda(
      ...
      : estemplate.lambda(
          ...
          makeBlock(
            mapBody,
            makeReturnArray(
              parentObj.nextParams.concat(mapBody.propagate.concat(ioFunc))
            )
          )
        );
...
const maybeBindStmt = (input) =>
  parser.all(
    bindIDParser,
    reverseBindParser,
    parser.any(fnCallParser, maybeDOMStmt, ioFuncName, nonReservedIdParser)
  )(input);
...
const formBindStmt = (
  ...
  let callBack = isEmptyArr(mapBody.stmts)
    ? estemplate.lambda(cbParams, estemplate.array(cbParams.concat(cbBody)))
    : estemplate.lambda(
        cbParams,
        makeBlock(
          mapBody,
          makeReturnArray(cbParams.concat(mapBody.propagate).concat(cbBody))
        )
      );
...
const maybeLetStmt = (input, parentObj, bindBody, mapBody) =>
  maybe(parser.all(letParser, letParamsIOParser)(input), (val, rest) => {
...
const ioBodyParser = (
  ...
  if (notNull(finalStmt))
    return maybeFinalStmt(finalStmt, input, parentObj, bindBody, mapBody);
...
const ioParser = (input) =>
  maybe(
    parser.all(
      nonReservedIdParser,
      equalSignParser,
      parser.any(doParser, noArgsCallParser, ioStmtParser, nonReservedIdParser)
    )(input),
    (initIO, rest) => {

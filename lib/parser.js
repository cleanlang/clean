...
*/
const biOps = require('./binaryOperators')
...
const lookAhead = src => {
  let match = parser.all(
    spaceParser,
    expressionParser,
    spaceParser,
    binaryOperatorParser, spaceParser)(src)
  if (match !== null) {
    let [[, expression, , operator], remaining] = match
    return [expression, operator, remaining]
  }
  return null
}
// Predifined regexes
...
const binaryExpressionParser = src => {
  if (parser.all(expressionParser, spaceParser)(src) !== null) {
    let [[leftOperand], rest] = parser.all(expressionParser, spaceParser)(src)
    if (binaryOperatorParser(rest) !== null) {
      let [firstOperator, right] = binaryOperatorParser(rest)
      if (lookAhead(right) !== null) {
        let [potentialRight, nextOperator, remaining] = lookAhead(right)
        let nextExp = parser.any(binaryExpressionParser, expressionParser)(remaining)
        return biOps[nextOperator].prec >= biOps[firstOperator].prec
                      ? estemplate.binaryExpression(firstOperator, leftOperand, estemplate.binaryExpression(nextOperator, potentialRight, nextExp))
                      : estemplate.binaryExpression(firstOperator, leftOperand, potentialRight)
      }
      return parser.all(spaceParser, expressionParser)(right)
    }
  }
  return null
}
// console.log(binaryExpressionParser('2 + 2 + 2 + 2 * 2'))

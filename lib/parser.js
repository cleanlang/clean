...
var spaceParser = input => mayBe(
  spaceRegex.exec(input.str),
  (a, space, rest) => returnRest(space, input, rest, {'name': 'column', 'value': space.length})
)
...
var multiLineParser = src => {
  const blockStmt = estemplate.blockStmt
  let stack = []
  let body = []
  const decide = source => {

    let result = parser.all(indentParser, statementParser)(source)

    if (result === null && stack.length !== 0) {
      body.push(blockStmt(body.splice(-stack.pop())))
      return [body, source]
    }

    if (result === null && stack.length === 0) return [body, source]

    let [[, stmt], nextLine] = result
    let {indent} = nextLine
    let nextIndent = indentParser(nextLine)[1].indent

    if (nextIndent > indent) {
      stack.push(1)
      body.push(stmt)
      return decide(nextLine)
    }

    if (nextIndent < indent) {
      body.push(blockStmt(body.splice(-stack.pop()).concat(stmt)))
  ...
  let [outAst, rest] = decide(src)
  return body.toString() === '' ? null : [outAst[0], rest]
}
...
var programParser = (input, ast = estemplate.ast()) => {
  let result = multiLineParser(input) || statementParser(input)

...
const { reservedFunctionCalls } = require('./languageConstructs')
...
const fnCallParser = (input) =>
  maybe(
    ...
    (val, rest) => {
      ...
      const callExpr = reservedFunctionCalls[callee.name]
...
const fnWithGuardsParser = (input) =>
  maybe(
    ...
    (val, rest) => {
      const [funcID, paramsArr, , body] = val
      return [estemplate.funcBlockDeclaration(funcID, paramsArr, body), rest]
    }
  )

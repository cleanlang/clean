...
const bindStatementParser = input => parser.all(bindIDParser, reverseBindParser, identifierParser, mayBeSpace, valueParser, mayBeNewLineAndIndent)(input)
...
const getIOBody = (input, parentObj = {}, thenBody = []) => {
  ...
  if (mayBeBind !== null) {
    ...
    if (isEmptyObj(parentObj)) {
      let val = estemplate.ioCall(ioFunc, [arg], nextParams)

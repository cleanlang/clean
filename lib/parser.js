...
var declParser = parser.bind(
  ...
  val => input => mayBe(
        parser.any(ifExprParser, binaryExprParser, fnCallParser, expressionParser, unaryExprParser)(input),
        (v, rest) => {
          return returnRest(estemplate.declaration(val, v, rest.line), input, rest.str)
        }
      )
)
...
var ifExprParser = input => mayBe(
  parser.all(
    ...
    (val, rest) => {
      ...
      return returnRest(estemplate.ifthenelse(condition, consequent, alternate, rest.line), input, rest.str)
    }
)

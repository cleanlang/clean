  ...
  if (finalStmt !== null) {
    ...
    if (result === null) {
      ...
      if (parentObj.returnVals !== undefined) {
        ...
        let cbBody = estemplate.array(thenBody.concat(returnVals))
...
const ioParser = input => {
  ...
  if (mayBeDo === 'do') {
    ...
    ioBody = mayBeDo.type === 'BlockStatement' ? mayBeDo : estemplate.defaultIOThen(mayBeDo)
  }

  ioBody.sType = 'IO'

  if (doID.name === 'main') {
    ioBody = ioBody.type === 'BlockStatement' ? ioBody : estemplate.blockStmt([ioBody])
    return returnRest(ioBody, input, rest.str)
  }

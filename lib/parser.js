...
var parenthesisParser = parser.bind(
    ...
    parser.bind(
      parser.bind(
        input => {
          ...
          return input.str.startsWith('(') ? returnRest('(', input, input.str.substr(1), {'name': 'column', 'value': 1}) : null
        },
        val => input => {
  ...
  val => input => input.str.startsWith(')') ? returnRest(val, input, input.str.substr(1), {'name': 'column', 'value': 1}) : null
)
...
var fnCallParser = parser.bind(
  ...
  val => input => mayBe(
    ...
    ? [estemplate.fnCall(val[0].expression, args), input, rest.str]
    : [estemplate.fnCall(val[0], args), input, rest.str]
  )
)

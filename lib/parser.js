...
const maybeFinalStmt = (finalStmt, input, parentObj, bindBody, mapBody) => {
  ...
  parentObj = mapIsNotEmpty ? makeMap(parentObj, mapBody) : parentObj
  if (notNull(maybeReturn)) {
    [[, , parentObj.returnVals], rest] = maybeReturn
  }
  return mapIsNotEmpty ? ioBodyParser(rest, parentObj, bindBody) : ioBodyParser(rest, parentObj, bindBody, mapBody)
}

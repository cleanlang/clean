...
const binaryExpressionParser = src => {
  let [[left], rest] = parser.all(expressionParser, spaceParser)(src)
  let [currentOp, right] =  binaryOperatorParser(rest)
  let lookAhead = parser.all(spaceParser, expressionParser)
    // spaceParser,
    // parser.any(binaryExpressionParser, expressionParser))(src)
  // if (result === null) { return null }
  // let [val, rest] = result
  // return [estemplate.binaryExpression(val), rest]
}

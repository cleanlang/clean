...
parser.regex = regex => src => mayBe(
  regex.exec(src.str),
  (m, val, rest) => returnRest(val, src, rest)
)
...
var numberParser = input => mayBe(
  ...
  (a, num, rest) => returnRest(estemplate.literal(num, num), input, rest)
)

var identifierParser = input => mayBe(
  idRegex.exec(input.str),
  (a, name, rest) => returnRest(estemplate.identifier(name), input, rest)
)
...
var parenthesisParser = parser.bind(
    ...
    parser.bind(
      parser.bind(
        input => {
          ...
          return input.str.startsWith('(') ? returnRest('(', input, input.str.substr(1)) : null
        },
        val => input => {
  ...
  val => input => input.str.startsWith(')') ? returnRest(val, input, input.str.substr(1)) : null
)
...
var declParser = parser.bind(
  ...
  val => input => mayBe(
        parser.any(binaryExprParser, expressionParser)(input),
        (v, rest) => {
...
var fnDeclParser = parser.bind(
  ...
  val => input => mayBe(
      parser.any(binaryExprParser, expressionParser, fnCallParser)(input),
      (v, rest) => {
  ...
  parser.bind(
    parser.bind(
      ...
      val => input => mayBe(
        ...
        (params, rest) => returnRest(estemplate.lambda(params), input, rest)
       )
      ),
    val => input => mayBe(
      thinArrowParser(input),
      (v, rest) => returnRest(val, input, rest)
    )
   ),
   val => input => mayBe(
     ...
     (v, rest) => {
       ...
       return returnRest(val, input, rest)
     }
   )
 )
  ...
  parser.bind(
    parser.bind(
      ...
      val => input => mayBe(
        ...
        (val1, val2, rest) => returnRest(estemplate.letExpression(val1, val2), input, rest)
     )
    ),
    val => input => mayBe(
      inParser(input),
...
var argsParser = (input, argArray = []) => {
  var arg = parser.bind(expressionParser, val => input => {
    const space = spaceParser(input)
    return space ? [val, space[1]] : [val, input]
  })(input)
  var end = parser.all(expressionParser, parser.any(returnParser, parenCheck))(input)
  if (arg === null && end === null) return argArray.length === 0 ? null : [argArray, input]
...
var statementParser = parser.any(indentParser, returnParser, printParser, ifExprParser, letExpressionParser, declParser, fnDeclParser, fnCallParser, lambdaParser)

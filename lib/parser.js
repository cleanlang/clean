...
const bindStatementParser = input => parser.all(identifierParser, spaceParser, reverseBindParser, identifierParser, mayBeSpace, stringParser)(input)

const makeDeclArray = (idArray, valArray) => {
  return idArray.map((id, index) => estemplate.declaration(id, valArray[index]))
}

const letStmtParser = input => {
  let result = parser.all(letParser, letParamsParser)(input)
  if (result === null) return returnRest([], input, input.str)
  let [[, [idArray, valArray]], rest] = result
  let declArray = makeDeclArray(idArray, valArray)
  return returnRest(declArray, input, rest.str)
}

const ioParser = input => {
  let result = parser.all(identifierParser, equalSignParser, doParser, mayBeNewLineAndIndent,
                          bindStatementParser, mayBeNewLineAndIndent,
                          letStmtParser, mayBeNewLineAndIndent,
                          fnCallParser)(input)

  if (result === null) return null
  let [[fnName, , , , [id, , , func, , strPath], , declArray, , callback], rest] = result
  let val = estemplate.funcDeclaration(fnName, [], estemplate.blockStmt([estemplate.ioDo, estemplate.io(id, estemplate.blockStmt(declArray.concat(callback)), strPath, func)]))
  val.declarations[0].init.expression = false
  // otherwise the blockstmt is taken as an object and throws an error
  return returnRest(val, input, rest.str)
}

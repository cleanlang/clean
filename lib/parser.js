...
var conditionParser = src => {
  let op = parser.all(parser.any(fnCallParser, identifierParser),
  spaceParser,
...
var binaryExprParser = src => {
  ...
  let result = parser.all(
                          parser.any(identifierParser, fnCallParser, expressionParser),
                          spaceParser,
...
var ifExprParser = parser.bind(
  ...
  parser.bind(
    ...
    val => str => mayBe(
      parser.any(fnCallParser, expressionParser, binaryExprParser)(str),
      (v, rest) => [[val, v], rest]
    )),
    val => str => mayBe(
...
var statementParser = parser.any(spaceParser, printParser, ifExprParser, letExpressionParser, declParser, fnDeclParser, fnCallParser, lambdaParser)

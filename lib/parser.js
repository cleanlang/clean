...
var multiLineParser = src => {
  src.str = src.str.split('\n')
  if (src.str.length > 1) {
    let indent = indentParser(src.str[1])
    if (!indent) {
      src.str = src.str.join('\n')
      return src
    }
    const lead = src.str.splice(0, 1)
    const follow = src.str.splice(0, 1)
    src.str.unshift(lead + follow)
    src.str = src.str.join('\n')
    return multiLineParser(returnRest('', src, src.str)[1])
  }
  src.str = src.str.join('\n')
  return src
}
...
var programParser = (input, ast = estemplate.ast()) => {
  let result = multiLineParser(input)
  result = statementParser(result)

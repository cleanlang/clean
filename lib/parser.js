...
const {
  ...
  reverseBindParser, doParser, mainParser, putLineParser, assignmentOperatorParser
} = base
...
const getIOBody = (input, parentObj = {}, thenBody = []) => {
  ...
  if (finalStmt !== null) {
    ...
    if (result === null) {
      if (thenBody.length === 0) return returnRest(estemplate.defaultIOThen(parentObj), input, rest.str)
      let cbParams = parentObj.nextParams
      let cbBody = estemplate.blockStmt(thenBody)
      let callBack = estemplate.lambda(cbParams, cbBody)
...
const ioParser = input => {
  ...
  val.sType = 'IO'
  return returnRest(val, input, _rest.str)
}

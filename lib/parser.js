...
const expressionParser = parser.any(identifierParser, numberParser, parenthesisParser)
...
const argParser = expressionParser

const argsParser = (str, argArray = []) => {
  const arg = parser.all(argParser, spaceParser)(str)
...
const statementParser = parser.all(
  spaceParser,
  parser.any(letExpressionParser, declParser, fnDeclParser, fnCallParser, lambdaParser)
)

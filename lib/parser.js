...
const bindStatementParser = input => parser.all(identifierParser, spaceParser, reverseBindParser, identifierParser, mayBeSpace, stringParser, mayBeNewLineAndIndent)(input)
...
const getIOBody = (input, parentObj = {}) => {
  let finalStmt = fnCallParser(input)
  if (finalStmt !== null) {
    let [funcBody, rest] = finalStmt
    let [cbBody, cbParams] = [estemplate.blockStmt([funcBody]), parentObj.nextParams]
    let callBack = estemplate.lambda(cbParams, cbBody)
    let val = estemplate.ioThen(parentObj, callBack)
    return returnRest(val, input, rest.str)
  }
  ...
  if (mayBeBind !== null) {
    ...
    let callBack = estemplate.lambda(cbParams, estemplate.blockStmt([cbBody]))
...
const ioParser = input => {
  ...
  let [ioBody, _rest] = getIOBody(rest, estemplate.ioCall(ioFunc, arg, [bindID]))
  ioBody.expression = false
  ioBody = estemplate.blockStmt([ioBody])

...
const binaryExprParser = (
  ...
  switch (expect) {
    case 'operand': {
      const maybeOperand = parser.any(fnCallParser, expressionParser)(input);
...
const valueAndWhereClauseParser = (input) =>
  maybe(
    ...
    (val, rest) => {
      ...
      block.body = [
        ...whereDecls,
        nonReturnStatements.includes(v.type) ? v : estemplate.returnStmt(v)
      ]
      return [block, rest]
    }
  )
...
const declParser = (input) =>
  maybe(
    ...
    (val, rest) => {
      const [id, , value] = val
      const nonReturnStatements = ['BlockStatement']
      return [
        nonReturnStatements.includes(value.type)
...
const guardsParser = (input) => {
  ...
  const defaultCase = maybe(
    ...
    (val, rest) => {
      ...
      return [estemplate.ifStmt(estemplate.boolLiteral('true'), defaultCase, true), rest]
    }
  )
  if (defaultCase) {
    guards.push(
...
const fnWithGuardsParser = (input) => {
  return maybe(
    ...
    (val, rest) => {
      const [funcID, paramsArr, , body] = val
      return [estemplate.funcDeclaration(funcID, paramsArr, estemplate.blockStmt([body])), rest]
    }
  )
}

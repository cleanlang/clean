...
var objectParser = input => {
  let openCurlyResult, closeCurlyResult, spaceResult, rest, keyResult, valResult, val, key, colonResult, commaResult, result
  let propArr = []
  if (!(openCurlyResult = openCurlyBraceParser(input))) return null
  rest = openCurlyResult[1]
  let objectPropParser = rest => {
    rest = (spaceResult = parser.any(spaceParser, returnParser)(rest)) ? spaceResult[1] : rest
    if (!(keyResult = parser.any(identifierParser, stringParser)(rest))) return [propArr, rest]
    key = keyResult[0]
    rest = keyResult[1]
    rest = (spaceResult = parser.any(spaceParser, returnParser)(rest)) ? spaceResult[1] : rest
    if (!(colonResult = colonParser(rest))) return null
    rest = colonResult[1]
    rest = (spaceResult = parser.any(spaceParser, returnParser)(rest)) ? spaceResult[1] : rest
    let requiredParser = [binaryExprParser, numberParser, stringParser, booleanParser]
    if (!(valResult = parser.any(...requiredParser)(rest))) return null
    val = valResult[0]
    rest = valResult[1]
    rest = (spaceResult = parser.any(spaceParser, returnParser)(rest)) ? spaceResult[1] : rest
    propArr.push(estemplate.objectProperty(key, val))
    if (!(commaResult = commaParser(rest))) return [propArr, rest]
    return objectPropParser(commaResult[1])
  }
  if (!(result = objectPropParser(rest))) return null
  rest = result[1]
  rest = (spaceResult = parser.any(spaceParser, returnParser)(rest)) ? spaceResult[1] : rest
  if (!(closeCurlyResult = closeCurlyBraceParser(rest))) return null
  return [estemplate.object(propArr), closeCurlyResult[1]]
}

...
const ioStmtParser = (input, parentObj) => maybe(
  ioFuncName(input),
  (val, rest) => {
    let [id, , args] = val
...
const letParamsIOParser = (str, letIdArray = [], letLiteralArray = []) => maybe(
...
const makeMapChain = (idArray, valArray, parentObj) => {
  ...
  let cbParams = parentObj.nextParams
  let val = isUndefined(valArray[0].expression) ? valArray[0] : valArray[0].expression
  let cbBody = estemplate.array([val].concat(cbParams))

...
const getIOBody = (input, parentObj = {}, thenBody = []) => {
  ...
  if (finalStmt !== null) {
    ...
    if (result === null) {
      ...
      let cbBody = estemplate.blockStmt(thenBody.concat(estemplate.returnStmt(cbParams)))
      let callBack = estemplate.lambda(cbParams, cbBody).expression
      let val = estemplate.ioMap(parentObj, callBack, cbParams)
      return returnRest(val, input, rest.str)
    }
    let [, _rest] = result
    return getIOBody(_rest, parentObj, thenBody)
  }
...
const ioParser = input => {
  ...
  if (doID.name === 'main') {
    ...
    let val = estemplate.lambdaCall([], [], ioBody)
    val.sType = 'IO'
    return returnRest(val, input, rest.str)
  }

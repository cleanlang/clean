...
var openSquareBracketParser = input => mayBe(
...
var closeSquareBracketParser = input => mayBe(
...
var paramParser = parser.all(spaceParser, parser.any(identifierParser, numberParser, stringParser))
...
var statementParser = parser.any(returnParser, declParser, ifExprParser, fnDeclParser, fnCallParser, lambdaParser)
...
const current = parser.any(unaryExprParser, binaryOperatorParser, spaceParser, fnCallParser, expressionParser)
...
var valueParser = parser.any(unaryExprParser, expressionParser, arrayParser, objectParser)
...
var arrayElemParser = input => mayBe(
  parser.all(objectSpaceParser, valueParser, objectSpaceParser)(input),
    (val, rest) => {
      let [ , value, , ] = val
      return [estemplate.array(value), rest]
    }
)

var arrayElemsParser = (input, propArr = []) => {
  let result = arrayElemParser(input)
  let commaResult
  if (result === null) {
    input = objectSpaceParser(input)[1]
    if ((commaResult = commaParser(input)) !== null) {
      propArr.push(null)
      return arrayElemsParser(commaResult[1], propArr)
    }
    return [propArr, input]
  }
  if (result === null) return [propArr, input]
  let [arrValAst, rest] = result
  propArr.push(arrValAst)
  if (!(commaResult = commaParser(rest))) return [propArr, rest]
  rest = commaResult[1]
  return arrayElemsParser(rest, propArr)
}

var arrayParser = input => {

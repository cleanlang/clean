...
const slashParser = parser.regex(/^(\\)((.|\n)*)$/)
...
const paramParser = parser.all(spaceParser, identifierParser)
...
const expressionParser = parser.any(fnCallParser, identifierParser, numberParser)
...
const binaryExprParser = src => {
  ...
  let result = parser.all(
    expressionParser, spaceParser,
    binaryOperatorParser,
    spaceParser, expressionParser)(src)
...
const statementParser = parser.any(spaceParser, declParser, fnDeclParser, fnCallParser)

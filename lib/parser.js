...
const getIOBody = (input, parentObj = {}, thenBody = []) => {
  ...
  if (mayBeBind !== null) {
    ...
    if (isEmptyObj(parentObj)) {
      let val = isIOCall ? estemplate.fnCall(mayBeIOFunc, []) : estemplate.ioCall(ioFunc, args, nextParams)
      val.nextParams = isIOCall ? nextParams : val.nextParams
      return getIOBody(rest, val, thenBody)
    }
    let cbBody = isIOCall ? estemplate.fnCall(mayBeIOFunc, []) : estemplate.ioCall(ioFunc, args, nextParams)

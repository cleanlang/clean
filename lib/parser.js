...
const {
  ...
  reverseBindParser, doParser, putLineParser, //, assignmentOperatorParser
  returnKeywordParser
} = base
...
const putLineStmtParser = (input, parentObj) => {
  ...
  if (isEmptyObj(parentObj)) {
    let val = estemplate.ioCall(estemplate.identifier('putLine'), args)
    val.sType = 'IO'
    return returnRest(val, input, rest.str)
  }
...
const getIOBody = (input, parentObj = {}, thenBody = []) => {
  ...
  if (finalStmt !== null) {
    ...
    if (result === null) {
      ...
      if (isEmptyObj(parentObj) && thenBody.length !== 0) {
        let val = estemplate.blockStmt(thenBody)
        val.expression = false
        return returnRest(val, input, rest.str)
      }

      let cbParams = parentObj.nextParams === undefined ? [] : parentObj.nextParams
      let returnVals = parentObj.returnVals !== undefined ? parentObj.returnVals : [estemplate.nullLiteral('null')]
      let cbBody = estemplate.blockStmt(thenBody.concat(estemplate.returnStmt(returnVals)))
  ...
  if (mayBeBind !== null) {
    ...
    let [ioFunc, , arg] = isIOCall ? [null, null, null] : mayBeIOFunc
    if (isEmptyObj(parentObj)) {
      let val = isIOCall ? estemplate.fnCall(mayBeIOFunc, []) : estemplate.ioCall(ioFunc, [arg], nextParams)
      val.nextParams = isIOCall ? nextParams : val.nextParams
      return getIOBody(rest, val, thenBody)
    }
    let cbBody = isIOCall ? estemplate.fnCall(mayBeIOFunc, []) : estemplate.ioCall(ioFunc, [arg], nextParams)
  ...
  if (mayBeIOCall !== null) {
    let [funcID, rest] = mayBeIOCall
    let funcCall = estemplate.fnCall(funcID, [])
    if (isEmptyObj(parentObj)) {
      funcCall.nextParams = []
    }
    let val = estemplate.expression(estemplate.defaultIOThen(funcCall))
    return getIOBody(rest, parentObj, thenBody.concat(val))
  }
...
const ioParser = input => {
  let initIO = parser.all(identifierParser, equalSignParser, parser.any(putLineStmtParser, doParser, identifierParser))(input)
  ...
  if (mayBeDo === 'do') {
    ...
    ioBody = mayBeDo.type === 'Identifier' ? estemplate.fnCall(mayBeDo, []) : mayBeDo
  }

  if (mayBeDo.type !== undefined && mayBeDo.type === 'Identifier' && doID.name !== 'main') return null
  if (doID.name === 'main') {
    ioBody = ioBody.type === 'BlockStatement' ? ioBody : estemplate.blockStmt([estemplate.defaultIOThen(ioBody)])
    let val = estemplate.blockStmt([estemplate.lambdaCall([], [], ioBody)])
    val.sType = 'IO'
    return returnRest(val, input, rest.str)
  }
  ...
  let val = estemplate.funcDeclaration(doID, [], ioBody)
  val.sType = 'IO'
  return returnRest(val, input, rest.str)
}

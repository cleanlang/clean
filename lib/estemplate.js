...
estemplate.ifthenelse = (condition, result1, result2, line) => {
  [condition, result1, result2] = [extractExpr(condition), extractExpr(result1), extractExpr(result2)]
  let expressions = [condition, result1, result2].filter(e => e.interrupt === true)
  if (expressions.length > 0) return expressions[0]
  let [res1Type, res2Type] = [result1, result2].map(checkIdentifier)
  if (res1Type !== res2Type) {
    return estemplate.error(`Conditional expressions must have results of the same type. Error at line: ${line + 2}`)
  }
  return {
    'type': 'ExpressionStatement',
    'expression': {

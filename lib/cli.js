...
const evalFunction = (argsObj, infile, outfile) => {
  ...
  const importCore = minify(fs.readFileSync(importPath + 'core.js', 'utf8').toString() +
...
const fileWatcher = (argsObj, outfile, changedInpfile) => {
  ...
  if (infilePathObj.ext === '.cl') {
    const infilePath = path.join(__dirname, `../${infilePathObj.dir}`, infilePathObj.base)
    const subFolder = infilePathObj.dir.split('/').slice(1).join('/')
    const outfolderPath = path.join(__dirname, `../${outfile}`, subFolder)
...
const createFolder = (outfile, changedInpfolder) => {
  ...
  const outFolder = path.join(__dirname, `../${outfile}`, subFolder)
  makeDir(outFolder)
}
...
const removeFile = (outfile, changedInpfile) => {
  ...
  if (rmvfilePathObj.ext === '.cl') {
    ...
    const outfilePath = path.join(__dirname, `../${outfile}`, subFolder, `${rmvfilePathObj.name}.js`)
    rmvFile(outfilePath)
  }
}

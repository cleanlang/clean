...
module.exports = (argsObj) => {
  ...
  const isWatchFolder = argsObj.w
  if (!inputArg && !isWatchFolder) {
  ...
  if (isWatchFolder) {
    watcher(inputArg)
      ...
      .on('add', (addedFile) =>
        fileWatcher(argsObj, outfile, addedFile)
      )
      .on('change', (changedFile) =>
        fileWatcher(argsObj, outfile, changedFile)
      )
      .on('unlinkDir', (unlinkedDir) =>
        removeFolder(outfile, unlinkedDir)
      )
      .on('unlink', (unlinkedFile) =>
        removeFile(outfile, unlinkedFile)
      )
  }
}

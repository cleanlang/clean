...
const matchArgTypesToAcceptedTypes = (args, acceptedTypes, localTypeObj, id) => {
  ...
  return _args.reduce((type1, type2) => type1 === type2)
}
...
const statementType = expr => {
  ...
  switch (type) {
    ...
    default:
      ...
      if (type === null) globalTypesObj['error'] = {flag: true, loc: expr.cursorLoc}
  }
}
...
const types = body => {
  ...
  let error = null
  if (globalTypesObj.error !== undefined && globalTypesObj.error.flag) return [errorTemplate(`TYPE ERROR: at line ${globalTypesObj.error.loc.line}, col ${globalTypesObj.error.loc.column}`)]
  for (let decl in globalTypesObj) {
    if (globalTypesObj[decl] === null) {
      error = errorTemplate(`TYPE ERROR: at ${decl}`)
      break
    }
  }

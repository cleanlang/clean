...
const callExprType = (expr, localTypeObj = {}, id) => {
  ...
  if (params !== undefined && body !== undefined && !(isEmptyObj(localTypeObj))) {
...
const parentIsArray = (parentId, property, localTypeObj) => {
  return !(isEmptyObj(localTypeObj)) && localTypeObj[parentId.name] !== undefined && (property.type === 'Identifier' || property.sType === 'number') && property.isSubscript
}

const parentIsObject = (parentId, property, localTypeObj) => {
  return !(isEmptyObj(localTypeObj)) && localTypeObj[parentId.name] !== undefined && ((property.type === 'Identifier' && !(property.isSubscript)) || (property.sType === 'string' && property.isSubscript))
}

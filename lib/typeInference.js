 ...
 * IO blocks are termed as type 'IO' and whenever the type could not be inferred,
 * it is set to 'needsInference' globalTypesObj keeps track of all identifiers
 * and their types, function declarations and their accepted and return types.
 */
...
const returnIdentifierType = (id, typeObj) => {
  if (isFunction(id.name, typeObj))
    return getParamReturnTypes(id, typeObj);
...
const unaryExprType = (expr) => {
  ...
  if (expr.operator === "!" && exprType(expr.argument) === "bool")
    return "bool";
...
const assignTypeToUninferredIdentifier = (
  expr,
  inferredType,
  localTypeObj
) => {
...
const conditionalExprType = (expr, localTypeObj, id) => {
  ...
  if (
    consequentType.type !== undefined &&
    alternateType.type !== undefined
  ) {
...
const matchArgTypesToAcceptedTypes = (
  ...
  return isEmpty(_args)
    ? true
    : _args.reduce((type1, type2) => type1 === type2);
};
...
const callExprType = (expr, localTypeObj = {}, id) => {
  ...
  if (
    ...
    const typesOfParams = mapArgTypeToParams(
      params,
      args,
      localTypeObj,
      id
  ...
  const isRecursive = checkForRecursion(
    expr.callee.name,
    id,
    localTypeObj,
    args
  ...
  if (expr.callee.id === null)
    result = [result.paramTypes, result.returnType];
...
const reduceTypes = (typesArr, localTypeObj, id) => {
  const reducedType = typesArr
    ...
    .reduce((exp1, exp2) => {
      ...
      if (
        typeof exp1.type === "object" &&
        typeof exp2.type === "object"
      ) {
...
const switchStatementType = (body, localTypeObj, id) => {
  ...
  return checkArgsType === false || checkTestType === false
    ? null
    : returnType;
};
...
const arrayExprType = (expr, localTypeObj, id) => {
  ...
  const deducedtype = expr.elements
    ...
    .reduce((type1, type2) =>
      type1 === type2 ? type1 : "needsInference"
    );
  const arrayType = {
    type: "array",
...
const objectExprType = (expr, localTypeObj, id) => {
  ...
  expr.properties.forEach((prop) => {
    if (prop.key.value === undefined) {
      propertyTypeObj[prop.key.name] = exprType(
        prop.value,
        localTypeObj
      );
    } else {
      propertyTypeObj[prop.key.value] = exprType(
        prop.value,
        localTypeObj
      );
    }
...
const getParentTypes = (propSpec, propName, parentId, localTypeObj) => {
  ...
  for (const type in propSpec) {
    if (propSpec[type][propName] !== undefined)
...
const getPropReturnType = (propSpec, propName) => {
  ...
  if (propSpec.isProp)
    return { type: propSpec.returnType, id: propName };
};

const checkForInbuiltProp = (
  parentType,
  prop,
  parentId,
  localTypeObj
  ...
  if (propName === null)
    return { type: "needsInference", id: propName };
  ...
  switch (propSpec.type) {
    ...
    case false: {
      if (parentObj.type === "object") {
        ...
        if (prop.sType !== undefined && prop.sType !== "string")
          return null;
        if (parentObj.propTypes === "needsInference")
          return "needsInference";
        return getTypeOfChild(
      ...
      if (parentObj.type === "array") {
        ...
        if (prop.sType === "number") {
          ...
          if (isEmptyObj(parentObj.elemTypes))
            return parentObj.commonType;
          return getTypeOfChild(

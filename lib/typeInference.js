...
const matchArgTypesToAcceptedTypes = (args, acceptedTypes, localTypeObj, id) => args.map(
  (arg, index) => exprType(arg, localTypeObj, id) === acceptedTypes[index])
...
const switchStatementType = (body, localTypeObj, id) => {
  ...
  let [returnType, acceptedType] = [
    ...
    initialPattern.test].map(e => exprType(e, localTypeObj, id))
...
const arrayExprType = (expr, localTypeObj, id) => {
  let arrayTypeObj = {}
  expr.elements.map((e, index) => { arrayTypeObj[index] = exprType(e, localTypeObj, id) })

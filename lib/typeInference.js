...
const matchArgTypesToAcceptedTypes = (args, acceptedTypes, localTypeObj, id) => (
  args.map((arg, index, arr) => {
    ...
    return type === acceptedTypes[index]
  }).reduce((type1, type2, index, arr) => type1 === type2))
...
const callExprType = (expr, localTypeObj = {}, id) => {
  if (expr.callee.id !== null && globalTypesObj[expr.callee.name] === undefined) return 'needsInference' // type inference disabled for inbuilt functions
...
const getTypeOfChild = (parentObj, propsArray, parentId, localTypeObj, id) => {
  ...
  switch (propSpec.type) {
    case 'needsInference': {
      if (parentIsArray(parentId, prop, localTypeObj)) {
        localTypeObj[parentId.name] = {
          'type': 'array',
          'elemTypes': null,
          'commonType': 'needsInference',
          'isHomogeneous': null
      ...
      if (parentIsObject(parentId, prop, localTypeObj)) {
        localTypeObj[parentId.name] = {
          'type': 'object',
          'propTypes': 'needsInference'
        }
      }
      return 'needsInference'
    }// support for later binary and function calls in subscript
